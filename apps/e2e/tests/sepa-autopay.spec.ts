...
import { api, createSentInvoice, login, stripeWebhookSecret } from "../api";
 ...
 * invoice paid. So this drives the real webhook endpoint with events signed exactly the way
 * Stripe signs them (HMAC-SHA256 over `timestamp.payload` with the endpoint secret), covering
 * signature verification, the raw-body plumbing, metadata routing and redelivery dedup together.
 */
function signedDelivery(event: object, secret = stripeWebhookSecret()) {
...
function paymentIntentEvent(type: "payment_intent.succeeded" | "payment_intent.payment_failed", intentId: string, meta: Record<string, string>, amountCents: number) {
  return {
...
async function deliver(event: object, secret?: string) {
  ...
  return fetch(`${apiUrl()}/billing/webhook`, {
    method: "POST",
...
test("a settled SEPA autopay debit marks the invoice paid exactly once, and a failed one leaves it open", async ({ page }) => {
  ...
  await test.step("an event with a forged signature is refused and changes nothing", async () => {
    const res = await deliver(paymentIntentEvent("payment_intent.succeeded", intentId, meta, amountCents), "whsec_not_the_real_secret");
  ...
  await test.step("a failed debit leaves the invoice sent", async () => {
    const res = await deliver(paymentIntentEvent("payment_intent.payment_failed", `${intentId}_failed`, meta, amountCents));
  ...
  await test.step("the settled debit records one bank-transfer payment for the full amount", async () => {
    const res = await deliver(paymentIntentEvent("payment_intent.succeeded", intentId, meta, amountCents));
    expect(res.status).toBe(201);
