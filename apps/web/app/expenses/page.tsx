...
export default function ExpensesPage() {
  ...
  function load() {
    apiFetch<Expense[]>(`/expenses?${queryString()}`).then((page) => {
      setExpenses(page);
      setHasMore(page.length === EXPENSES_PAGE_SIZE);
    });
  }
  ...
  async function loadMore() {
    ...
    try {
      const page = await apiFetch<Expense[]>(
        `/expenses?${queryString(expenses[expenses.length - 1].id)}`,
      );
      setExpenses([...expenses, ...page]);
      setHasMore(page.length === EXPENSES_PAGE_SIZE);
    } finally {
  ...
  async function reject(id: string) {
    ...
    setBusy(true);
    try {
      await apiFetch(`/expenses/${id}/reject`, {
        method: "POST",
    ...
    <AuthenticatedShell>
      ...
      {totalPending > 0 && (
        <p className="mt-3 text-sm text-warning-700 dark:text-warning-500">
          {t("pendingTotal", { amount: totalPending.toFixed(2), currency })}
        </p>
      )}
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        ...
                        <span className="rounded-full bg-gray-100 dark:bg-gray-700 px-2 py-0.5 text-xs text-gray-600 dark:text-gray-300">
                          {EXPENSE_CATEGORIES.includes(e.category as never)
                            ? t(e.category)
                            : e.category}
                        </span>
                        {e.anomaly.isAnomaly && (
                          <span
                            ...
                            title={t("anomalyHint", {
                              average: e.anomaly.historicalAverage ?? 0,
                              currency,
                            })}
                          >
                            {t("anomaly", {
                              percent: e.anomaly.deviationPercent ?? 0,
                            })}
                          </span>
                        )}
                      </div>
