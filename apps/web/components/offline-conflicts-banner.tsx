...
export function OfflineConflictsBanner({ items, onChange }: { items: QueuedMutation[]; onChange: () => void }) {
  ...
  return (
    <div className="border-b border-error-200 bg-error-50 px-4 py-3">
      <p className="text-xs font-medium text-error-700">{t("syncConflicts", { count: items.length })}</p>
      <ul className="mt-2 flex flex-col gap-2">
        {items.map((item) => (
          <li key={item.id} className="rounded-lg border border-error-200 bg-white px-3 py-2 text-xs">
            <div className="font-medium text-gray-700">{item.kind}</div>
            <div className="mt-0.5 text-error-600">{item.error}</div>
            <div className="mt-2 flex gap-2">
              <button
                onClick={() => retry(item.id)}
                disabled={busyId === item.id}
                className="rounded-md bg-gray-100 px-2 py-1 font-medium text-gray-700"
              >
                {t("retrySync")}
              </button>
              <button
                ...
                disabled={busyId === item.id}
                className="rounded-md bg-error-100 px-2 py-1 font-medium text-error-700"
              >
                {t("discardSync")}
              </button>
            </div>
