            ...
            <li key={r.id} className="card">
              <div className="flex items-start justify-between gap-3">
                <div>
                  ...
                  <p className="mt-1 text-xs text-gray-500">{t("submittedBy", { name: r.submittedByClient.name })}</p>
                  <p className="mt-1.5 text-xs text-gray-500">{r.description}</p>
                  {r.convertedChangeOrder && <p className="mt-1.5 text-xs text-success-700">{t("linkedTo", { number: r.convertedChangeOrder.number })}</p>}
                  {r.status === "declined" && r.reviewNote && <p className="mt-1.5 text-xs text-gray-500">{t("declineReason", { note: r.reviewNote })}</p>}
                </div>
              </div>
                  ...
                  {convertingId === r.id ? (
                    <div className="flex items-end gap-2">
                      <label className="flex flex-1 flex-col gap-1 text-xs">
                        <span className="font-medium text-gray-700">{t("changeOrderId")}</span>
                    ...
                    <div className="flex items-end gap-2">
                      <label className="flex flex-1 flex-col gap-1 text-xs">
                        <span className="font-medium text-gray-700">{t("declineNoteLabel")}</span>
