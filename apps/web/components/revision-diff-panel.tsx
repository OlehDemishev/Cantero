...
export function RevisionDiffPanel({ estimateId, revisions, currency }: { estimateId: string; revisions: RevisionSummary[]; currency: string }) {
  ...
  return (
    <div className="mt-6 border-t border-gray-100 pt-4">
      <h3 className="mb-2 text-xs font-semibold text-gray-500">{t("compareRevisions")}</h3>
      ...
      {diff && (
        <div className="mt-3">
          ...
          {diff.added.length === 0 && diff.removed.length === 0 && diff.changed.length === 0 ? (
            <p className="text-sm text-gray-400">{t("noDifferences")}</p>
          ) : (
            <ul className="flex flex-col gap-1 text-sm">
              {diff.added.map((l) => (
                <li key={`add-${l.rateCatalogItemCode}`} className="text-success-700">
                  + {l.rateCatalogItemName} ({l.quantity} {l.unit})
                </li>
              ))}
              {diff.removed.map((l) => (
                <li key={`rem-${l.rateCatalogItemCode}`} className="text-error-700">
                  − {l.rateCatalogItemName} ({l.quantity} {l.unit})
                </li>
              ))}
              {diff.changed.map((l) => (
