...
const STATUS_STYLES: Record<CommissioningSystemStatus, string> = {
  pending_testing: "bg-gray-100 text-gray-600",
  ...
  return (
    <div className="mt-10">
      ...
      {!systems ? (
        ...
        <p className="text-sm text-gray-400">{t("noSystems")}</p>
      ) : (
              ...
              <li key={sys.id} className="card">
                <button onClick={() => setExpandedId(expanded ? null : sys.id)} className="flex w-full items-center justify-between text-left">
                  <span className="text-sm font-medium text-gray-900">
                    {sys.name}
                    {sys.category && <span className="ml-1.5 text-xs text-gray-400">({sys.category})</span>}
                  </span>
                ...
                {expanded && (
                  <div className="mt-3 flex flex-col gap-4 border-t border-gray-100 pt-3">
                    <div>
                      <h3 className="mb-1.5 text-xs font-semibold uppercase tracking-wide text-gray-500">{t("checklist")}</h3>
                        ...
                        <ul className="mb-2 flex flex-col gap-1">
                          {sys.checklistItems.map((item) => (
                            <li key={item.id} className="flex items-center gap-2 text-sm">
                              ...
                              <span className={item.done ? "text-gray-400 line-through" : "text-gray-700"}>{item.description}</span>
                            </li>
                          ))}
                        </ul>
                      )}
                    ...
                    <div>
                      <h3 className="mb-1.5 text-xs font-semibold uppercase tracking-wide text-gray-500">{t("functionalTests")}</h3>
                    ...
                    <div>
                      <h3 className="mb-1.5 text-xs font-semibold uppercase tracking-wide text-gray-500">{t("ownerTraining")}</h3>
                            ...
                            <li key={session.id}>
                              <div className="flex items-center justify-between">
                                ...
                                {session.ownerSignedOffAt ? (
                                  <span className="text-success-700">{t("signedOffBy", { name: session.ownerSignerName ?? "" })}</span>
                                  ...
                                  <button onClick={() => setSigningId(session.id)} className="text-brand-700 hover:underline">
                                    {t("signOff")}
                                  </button>
                                )}
                              </div>
